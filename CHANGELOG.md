...
### Security (OWASP-aligned hardening)
- CRITICAL: New `core/runtime_security.py` — hardened input normalization with
  bounded URL decode (depth 5), bounded Base64 decode (budget 24), safe JSON/hex
  unescape, and entropy quarantine (threshold 5.2). Replaces unbounded recursive
  decode in `bridge/utils.py`.
- CRITICAL: Semantic intent classifier with 5 policy categories
  (malicious_capability, data_exfiltration, privilege_escalation, tool_abuse,
  policy_evasion). Detects paraphrases, euphemisms, roleplay, and coercive
  instruction patterns. Fails closed when confidence is uncertain.
- CRITICAL: New `core/decision_store.py` — distributed replay defense with
  SHA256 decision tokens bound to request_id, timestamp, policy_version, and
  manifest_hash. NX-based idempotent claims with SQLite local fallback and
  Upstash Redis centralized store.
- CRITICAL: Fail-closed degraded mode — privileged actions denied when Upstash
  or remote dependencies are unavailable. Only explicitly safe read-only paths
  allowed. Fallback state recorded in every receipt.
- HIGH: Manifest version and expiry validation — `security_policy.json` now
  requires `version` and `expires_at` fields. Stale, expired, or
  version-mismatched manifests are hard-rejected. Key rotation support via
  `key_version` field.
- HIGH: Deployment drift detection — workers verify they use the same signed
  policy bundle. Mismatched manifest hash or policy version across instances
  triggers a hard deny.
- HIGH: Audit receipts now bound to request_id, policy_version, manifest_hash,
  fallback_state, and decision_token with HMAC signing.
- HIGH: Identity anchor persistence — replay token tracking survives restarts
  via `ALETHEIA_ANCHOR_STATE_PATH` env var.
- MEDIUM: Strict Pydantic schema validation with `extra="forbid"` and regex
  allowlists for structured fields.
- MEDIUM: Sovereign relay now fails closed on anchor errors (was fail-open).

### Added
- `core/runtime_security.py` — centralized hardened normalization, intent
  classifier, and schema validation
- `core/decision_store.py` — distributed replay defense and drift detection
- `tests/test_runtime_security_layer.py` — 25 tests: normalization regression,
  semantic intent, adversarial deny/allow, property-based idempotence (Hypothesis)
- `tests/test_distributed_security_integration.py` — 8 tests: replay rejection,
  restart recovery, partial deployment drift
- Degraded-mode fail-closed tests in `test_security_hardening_v2.py`
- Property-based tests using Hypothesis for normalization idempotence and
  decode bounds
- `httpx` added to main dependencies

### Changed
- `bridge/utils.py` — rewritten as thin facade delegating to
  `core.runtime_security.normalize_untrusted_text()`
- `bridge/fastapi_wrapper.py` — full pipeline rewrite: schema validation →
  bundle drift → rate limit → normalize → degraded gate → intent classify →
  replay claim → sandbox → scout/nitpicker/judge → audit
- `core/audit.py` — receipts extended with request_id, policy_version,
  manifest_hash, fallback_state, decision_token
- `manifest/signing.py` — version/expiry/key_version metadata validation,
  signature length check, crash-fast on verification failure
- `manifest/security_policy.json` — added `expires_at` field, re-signed
- `proximity/identity_anchor.py` — replay token tracking, state persistence,
  extended receipt metadata
- `proximity/sovereign_relay.py` — fail-closed anchor errors, receipt metadata
  population from action metadata
- `core/rate_limit.py` — `backend` and `degraded` attributes exposed
- Test count: 527 → 548 (21 new tests)
