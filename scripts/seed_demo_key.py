...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--dry-run", action="store_true", help="Don't write to KeyStore")
    parser.add_argument("--name", default="hosted-demo", help="Key label (default: hosted-demo)")
    ...
    if not raw_key:
        print("ERROR: ALETHEIA_DEMO_API_KEY is not set in the environment.", file=sys.stderr)
        return 2
    ...
    if existing is not None:
        print(f"KeyStore already contains demo key id={existing.id} prefix={existing.key_prefix}")
        return 0
    ...
    print(f"Imported demo key id={record.id} prefix={record.key_prefix} plan={record.plan}")
    return 0
