...
| Legacy receipt compatibility | `ALETHEIA_RECEIPT_SECRET` (+ `ALETHEIA_REQUIRE_ED25519_RECEIPTS=false` opt-out) | Verification of older HMAC receipts. **Since v2.0.0, HMAC receipts are rejected by default**. Migration recipe: (1) generate an Ed25519 keypair and set `ALETHEIA_RECEIPT_PRIVATE_KEY`/`_PUBLIC_KEY`; (2) set `ALETHEIA_REQUIRE_ED25519_RECEIPTS=false` so any in-flight pre-migration HMAC receipts still verify (new ones will be Ed25519); (3) once all pre-migration receipts have aged out, unset `ALETHEIA_REQUIRE_ED25519_RECEIPTS` (default `true` resumes) and remove `ALETHEIA_RECEIPT_SECRET`. Startup now accepts Ed25519 keys as substitute signing material — `ALETHEIA_RECEIPT_SECRET` is no longer required once Ed25519 keys are configured. |
