...
**Audit Cadence:**
- Red-team adversarial testing: 174 cases gated on every PR via the
  [`adversarial-tests` CI job](https://github.com/holeyfield33-art/aletheia-core/actions/workflows/ci.yml?query=branch%3Amain) —
  128 in [`tests/test_redteam_adversarial.py`](tests/test_redteam_adversarial.py),
  30 in [`tests/test_redteam_fixes.py`](tests/test_redteam_fixes.py),
  16 in [`tests/test_redteam_hardening.py`](tests/test_redteam_hardening.py).
  Run with stubs disabled (`ALETHEIA_DISABLE_TEST_STUBS=true`) so the suite
  exercises the real semantic layer in active mode.
- Static + dependency scanning: every PR via Bandit, Semgrep, CodeQL,
  pip-audit, npm audit. See "Automated Scanning" below.
- Supply-chain signal: OpenSSF Scorecard runs weekly; SBOMs and SLSA build
  provenance attestations published on every release.
- Code review: Two-maintainer approval for security-critical changes.
- Third-party audit: planned annually (see schedule below).

**Planned Audits:**
- Q1 2027: Full threat-model audit by external security firm
- Q3 2027: Dependency supply-chain audit

**Reproducible artifacts:** Every claim above maps to a workflow run page;
no metric is reported here that isn't backed by a clickable CI artifact.

## Automated Scanning

| Tool                      | Scope                      | Trigger                    | Workflow                                  |
| ------------------------- | -------------------------- | -------------------------- | ----------------------------------------- |
| Bandit                    | Python SAST                | every PR + weekly cron     | `.github/workflows/security.yml`          |
| Semgrep                   | Multi-language SAST        | every PR + weekly cron     | `.github/workflows/security.yml`          |
| CodeQL                    | Python + JS/TS, `security-extended` | every PR + weekly cron | `.github/workflows/codeql.yml`     |
| pip-audit                 | Python dependency CVEs     | every PR + weekly cron     | `.github/workflows/security.yml`          |
| npm audit                 | Node dependency CVEs       | every PR + weekly cron     | `.github/workflows/security.yml`          |
| Dependency hash check     | Verifies `--require-hashes`| every PR                   | `.github/workflows/security.yml`          |
| OpenSSF Scorecard         | Supply-chain posture       | weekly cron + push to main | `.github/workflows/scorecard.yml`         |
| Adversarial test suite    | 174 red-team cases         | every PR                   | `.github/workflows/ci.yml` (`adversarial-tests` job) |

All jobs above are required and hard-fail on regression — none are advisory.

## Software Bill of Materials (SBOM)

Every published release ships CycloneDX SBOMs as release assets:

- `sbom-python.cdx.json` — Python runtime dependency graph
- `sbom-node.cdx.json` — Node/frontend dependency graph

Download from any release page or via the CLI:

```sh
gh release download <tag> --pattern 'sbom-*.cdx.json' \
  --repo holeyfield33-art/aletheia-core
```

SBOMs are generated from the pinned `requirements.txt` (hash-verified) and
`package-lock.json`, so the graph matches what CI installs and what end-users
pull.

## Build Provenance (SLSA)

Release artifacts carry [SLSA build provenance attestations](https://slsa.dev/spec/v1.0/provenance)
generated by `actions/attest-build-provenance`. Verify a downloaded artifact
matches its CI build record with:

```sh
gh attestation verify <artifact> --repo holeyfield33-art/aletheia-core
```

Attestations are publicly browsable at
<https://github.com/holeyfield33-art/aletheia-core/attestations>.

**Last Completed:**
- v2.0.0 red-team gate: 174-test suite required-pass on every merge to `main`.
  Run history and per-commit logs:
  <https://github.com/holeyfield33-art/aletheia-core/actions/workflows/ci.yml?query=branch%3Amain>
