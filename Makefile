...
verify-launch:
	...
	grep -q '"1.6.0"' lib/site-config.ts && echo "site-config.ts: OK" || echo "site-config.ts: MISMATCH"
	grep -q '1.6.0' package.json && echo "package.json: OK" || echo "package.json: MISMATCH"
	@echo ""
	...
	if [ "$$count" -eq 588 ]; then echo "Test count: OK (588)"; else echo "Test count: MISMATCH (expected 588, got $$count)"; fi
	@echo ""
